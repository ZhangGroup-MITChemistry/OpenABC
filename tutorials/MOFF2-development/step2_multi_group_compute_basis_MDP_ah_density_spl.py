...
parser.add_argument('--n0', type=int, default=10000, help='The target number of noise samples used for training')
parser.add_argument('--n1', type=int, default=10000, help='The target number of data samples used for training')
...
all_MDPs = ['THB-C2', 'Ub2', 'Ub3', 'Gal3', 'hnRNPA1-star', 'FPs-GS8', 
            'FPs-GS16', 'SH4UD-SH3-SH2', 'TDP43_WtoA', 'D12', 'D23', 
            'D34', 'SMAD4']
assert args.protein in all_MDPs

MDP_parameters = pd.read_csv('/orcd/data/binz/001/congwang/TW-PCCG-develop/TW-PCCG-develop/compare-cg-simulations/MDP_parameters.csv')
MDP_parameters = MDP_parameters.set_index('name')

output_dict = {}

T1 = MDP_parameters.loc[args.protein, 'T (K)'] * unit.kelvin
...
MDP_noise_main_dir = '/orcd/data/binz/001/congwang/TW-PCCG-develop/TW-PCCG-develop/noise-cg-simulations/noise-TWPCCG-multi-v1-MDP'
unbiased_system_xml = f'{MDP_noise_main_dir}/{args.protein}-TWPCCG-multi-v1-MDP/unbiased-system/system.xml'

def get_custom_nb_exclusions(system):
    exclusions = []
    custom_nb_forces = [f for f in system.getForces() if isinstance(f, mm.CustomNonbondedForce)]
...
exclusions = np.array(get_custom_nb_exclusions(unbiased_system)).astype(int)
...
dielectric = 80.0
ionic_strength = MDP_parameters.loc[args.protein, 'I (mM)'] * unit.millimolar
hps_model = HPSModel()
...
# -------------------------------------------------------
# Gaussian basis (correct call for your function)
# -------------------------------------------------------
